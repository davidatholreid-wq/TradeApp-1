...
# These live in the top-of-file imports (rather than mid-file, near the
# route groups that use them) because `allocate_unique_code` and
# `validate_sa_id` are also called from auth/registration flows that
# run BEFORE the extracted Kredo route module is loaded.
...
# ============ Auth routes ============
# Route handlers extracted 2026-08-09 → /app/backend/routes/auth.py.
# Includes: register, login, forgot-password, reset-password,
# /auth/me (GET + PATCH), and /referral/lookup.
...
# constants remain here because they are still called by other
# non-rewards code paths in this module.
...
# Route handlers extracted 2026-08-09 → /app/backend/routes/kredo.py.
# The service-layer imports (`get_kredo_client`, `KredoAPIError`,
# `validate_sa_id`, `allocate_unique_code`) moved to the top of this
# file because they are also used by the auth/register flows above.


# ============ Cover Offers (Pricing Agents) ============
# Route handlers extracted 2026-08-09 → /app/backend/routes/cover.py.
# `COVER_OFFER_COST_ZAR`, `_sanitise_sub_for_pricing_agent` and
# `require_pricing_agent` are re-imported at the bottom of this
# file (see `include_router` block) because `_sanitise_sub_for_pricing_agent`
# is still called from the submission-view code above.
...
from routes.kredo import router as kredo_router, _ensure_market_values  # noqa: F401 — used by submissions code above
from routes.cover import (  # noqa: F401 — _sanitise_sub_for_pricing_agent used by submissions code above
